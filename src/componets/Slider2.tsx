  ...
  return (
    <div className=" lg:max-w-[1200px]">
        <div style={{ backgroundImage: `url(${slider[currentIndex]})` }}
          className="relative h-[500px] w-full bg-center bg-cover duration-500 rounded-2xl">
        ...
        <div className='w-full inline-flex'>
          {slider.map((slides,index)=>index<4 && (
            <div key={index} style={{ backgroundImage: `url(${slider[index]})`}} className='w-44 h-12 md:h-24 bg-cover rounded-2xl  m-1 md:m-2'>
            </div>
          ))}
          <div className='w-20 md:w-40 h-12 md:h-24 border-2 border-gray-300 bg-gray-400 rounded-2xl text-center flex justify-center items-center m-1 md:m-2'>
            ...
            <span className='text-sm md:text-xl'>+{slider.length - 4} more</span>
          </div>
        </div>
    </div>
  )
